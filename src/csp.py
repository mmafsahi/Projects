...
# For HW1 - same as backtracking_search, but this also returns the number of
# variable assignments that occurred in this search.
def backtracking_search_with_assigment_count(csp,
                        select_unassigned_variable=first_unassigned_variable,
                        order_domain_values=unordered_domain_values,
                        inference=no_inference):
    """[Figure 6.5]"""

    def backtrack(assignment, num_assignments):
        if len(assignment) == len(csp.variables):
            return (assignment, num_assignments)
        var = select_unassigned_variable(assignment, csp)
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                csp.assign(var, value, assignment)
                num_assignments += 1        # accumulating total number of assignments - required for HW1
                removals = csp.suppose(var, value)
                if inference(csp, var, value, assignment, removals):
                    result, count = backtrack(assignment, num_assignments)
                    if result is not None:
                        return result, count
                csp.restore(removals)
        csp.unassign(var, assignment)
    ...
    result, num_assignments = backtrack({}, 0)
    assert result is None or csp.goal_test(result)
...
# For HW1 - same as min_conflicts, but also returns the number of 
# variable assignments that occurred in this search
def min_conflicts_with_num_assignments(csp, max_steps=100000):
    """Solve a CSP by stochastic Hill Climbing on the number of conflicts."""
    # Generate a complete assignment for all variables (probably with conflicts)
    csp.current = current = {}
    for var in csp.variables:
